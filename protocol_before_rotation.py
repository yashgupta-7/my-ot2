...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Load a tiprack in slot 1
    tiprack = protocol.load_labware(tiprack_used, '1')

    # Load a P300 Multi GEN2 on the right mount
    left_pipette = protocol.load_instrument(
         lft_pipette, 'left', tip_racks=[tiprack])

    right_pipette = protocol.load_instrument(
         rgt_pipette, 'right', tip_racks=[tiprack])

    # commands
    s = 1
    for sp in enumerate(src_plts):
        for i in range(1, 9):
